...
sum_1880 <- df_1880 %>% group_by(lon,lat,Acronym) %>%  dplyr::summarise(AI.avg = mean(AI, na.rm = T), AI.sd = sd(AI, na.rm = T)) %>% as.data.frame()
sum_1880$cat.AI <- sum_1880$AI.avg %>% cut(breaks.martonne, include.lowest = T) %>% revalue(cat.martonne)

ggplot(sum_1880)+geom_raster(aes(x=lon, y = lat, fill = cat.AI))+
...
ggplot(sum_1984)+geom_raster(aes(x=lon, y = lat, fill = cat.AI))+
    scale_fill_manual(values = col.cat, na.value = "grey90")+  labs(title = "AI 1984-2014", fill = "")+
    theme_void(base_size = 10)+theme(legend.position = "bottom")+guides(fill = guide_legend(nrow = 2))
