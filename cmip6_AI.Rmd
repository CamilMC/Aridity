...
era5 <- mutate(era5, pr = mpr, Rn = - (msshf + mslhf), rsds = msnswrf)

wdc <- read.table("wdc.txt")

wdc <- mutate(wdc, 
              Rn = rsds / 1000 * 60 * 60 *24, # conversion from kJ/m2/day to W/m2,
              mpr = pr / (60*60*24*365), # conversion from mm/y to kg/m2/s
              tas = tas + 273.15, # °C to K
              tas_min = tmin + 274.15)
...
vars <-  c("lon","lat","model","period","lm", "Continent","Type","Name","Acronym","source","tas","tas_min","pr","Rn","sfcWind","z")
...
ggplot(subset(clim, period == "1970_2000")) + geom_boxplot(aes(x= source, y = Rn))
ggplot(subset(clim, period == "1970_2000")) + geom_boxplot(aes(x= source, y = sfcWind))

ggplot()+
  geom_boxplot(data = subset(clim, period == "1970_2000" & source == "ERA5"), aes(x = "rsds ERA", y = rsds))+
  geom_boxplot(data = subset(clim, period == "1970_2000" & source == "ERA5"), aes(x = "Rn ERA", y = Rn))+
  geom_boxplot(data = subset(clim, period == "1970_2000" & source == "Worldclim"), aes(x = "rsds Worldclim", y = rsds))+
  theme_minimal()
```

AIm = De Martonne Aridity index 
...
AIu = UNESCO Aridity index

```{r aridity-index-unesco}

clim$Rn <- clim$Rn * 86400 * 1e-6 # conversion from W/m2 to MJ/m2/day.
...
clim$ET0 <-with(clim, ((0.408*delta*Rn+gamma*900/((tas-273.15)+273)*0.748*sfcWind*(es-ea))/(delta+gamma*(1+0.34*0.748*sfcWind)))) # Evapotranspiration, G considered to be negligible. in mm/day. Wind speed at 10 m is converted to wind speed at 2 m by multiplying by 0.748
...
```{r save-wdc}
write.table(clim, "cmip6_AI/clim.txt")
```
...
```{r load-clim-data}
clim <- read.table("cmip6_AI/clim.txt")
...
cmip6_ref <- clim %>% filter(period == "1970_2000" & source != "Worldclim" & source != "ERA5") %>%
   group_by(lon, lat, Name, Acronym) %>% 
  dplyr::summarise(Rn = mean(Rn, na.rm = T)) %>%
  mutate(source = "CMIP6")

ref <- clim %>% subset(period == "1970_2000" & source %in% c("Worldclim", "ERA5")) %>% 
       select(c("lon","lat", "Name","Acronym", "source","Rn"))
     
calib <- rbind(cmip6_ref, ref)

ggplot(data = subset(calib, source == "Worldclim")) + geom_raster(aes(x=lon, y = lat,  fill = Rn))+
  scale_fill_continuous_divergingx(palette = "RdYlBu", rev = T, na.value = "black", limits = c(-10,30))+
    labs(title = "Wordclim, 1970-2000", fill = "")+
    theme_void()+
    theme(legend.position = "bottom")

ggplot(data = subset(calib, source == "ERA5")) + geom_raster(aes(x=lon, y = lat,  fill = Rn))+
  scale_fill_continuous_divergingx(palette = "RdYlBu", rev = T, na.value = "black", limits = c(-10,30))+
    labs(title = "ERA5, 1970-2000", fill = "")+
    theme_void()+
    theme(legend.position = "bottom")

ggplot(data = subset(calib, source == "CMIP6")) + geom_raster(aes(x=lon, y = lat,  fill = Rn))+
  scale_fill_continuous_divergingx(palette = "RdYlBu", rev = T, na.value = "black", limits = c(-10,30))+
    labs(title = "CMIP6, 1970-2000", fill = "")+
    theme_void()+
    theme(legend.position = "bottom")
...
cmip6_ref <- clim %>% filter(period == "1970_2000" & source != "Worldclim" & source != "ERA5") %>%
   group_by(lon, lat, Name, Acronym) %>% 
  dplyr::summarise(rsds = mean(rsds, na.rm = T)) %>%
  mutate(source = "CMIP6")

ref <- clim %>% subset(period == "1970_2000" & source %in% c("Worldclim", "ERA5")) %>% 
       select(c("lon","lat", "Name","Acronym", "source","rsds"))
     
calib <- rbind(cmip6_ref, ref)

ggplot(data = subset(calib, source == "Worldclim")) + geom_raster(aes(x=lon, y = lat,  fill = rsds))+
  scale_fill_continuous_divergingx(palette = "RdYlBu", rev = T, na.value = "black", limits = c(-10,450))+
    labs(title = "Wordclim, 1970-2000", fill = "")+
    theme_void()+
    theme(legend.position = "bottom")

ggplot(data = subset(calib, source == "ERA5")) + geom_raster(aes(x=lon, y = lat,  fill = rsds))+
  scale_fill_continuous_divergingx(palette = "RdYlBu", rev = T, na.value = "black", limits = c(-10,450))+
    labs(title = "ERA5, 1970-2000", fill = "")+
    theme_void()+
    theme(legend.position = "bottom")

ggplot(data = subset(calib, source == "CMIP6")) + geom_raster(aes(x=lon, y = lat,  fill = rsds))+
  scale_fill_continuous_divergingx(palette = "RdYlBu", rev = T, na.value = "black", limits = c(-10,450))+
    labs(title = "CMIP6, 1970-2000", fill = "")+
    theme_void()+
    theme(legend.position = "bottom")
...
for(i in unique(clim$source)){
  plottitle <- paste(i, "historical, 1970-2000", sep = ", ")
  g <- ggplot(data = filter(clim, period == "1970_2000" & source == i)) + geom_raster(aes(x=lon, y = lat,  fill = tas-273.15))+
    scale_fill_continuous_divergingx(palette = "RdYlBu", rev = T, mid = 0, limits = c(-60,40))+
    labs(title = plottitle, fill = "")+
    theme_void()+
    theme(legend.position = "bottom")
