...
wdc <- wdc %>% mutate(psi = pi/180*lat, # latitude in radian
                    ...
                    Rs.Rso_02 = ifelse((Rs_02)/Rso_02 < 1, Rs_01/Rso_02, 1),
                    Rs.Rso_03 = ifelse((Rs_03)/Rso_03 < 1, Rs_01/Rso_03, 1),
...
wdc$mET0 <-with(wdc,((0.408*delta*Rn+gamma*900/((tas)+273)*0.748*sfcWind*(es-ea))/(delta+gamma*(1+0.34*0.748*sfcWind))))
wdc$mAI <- with(wdc, pr/(mET0*365))
...
wdc$AI2 <- rowMeans(select(wdc,c("AI_01", "AI_02", "AI_03", "AI_04", "AI_05", "AI_06", "AI_07", "AI_08", "AI_09", "AI_10", "AI_11", "AI_12")), na.rm = T)
...
ggplot() + geom_tile(data = wdc, aes(x=lon, y = lat,  fill = cat.mAI))+
  scale_fill_manual(values = col.cat)+
  theme_void()+
  theme(legend.position = "bottom")

ggplot() + geom_tile(data = wdc, aes(x=lon, y = lat,  fill = cat.AI))+
  scale_fill_manual(values = col.cat)+
  theme_void()+
  theme(legend.position = "bottom")

ggplot() + geom_tile(data = wdc, aes(x=lon, y = lat,  fill = cat.AI2))+
  scale_fill_manual(values = col.cat)+
  theme_void()+
  theme(legend.position = "bottom")
```
