...
## Aridity index with Thornthwaite 

[1] "lon"        "lat"        "tas"        "tas_01"    
  [5] "tas_02"     "tas_03"     "tas_04"     "tas_05"    
  [9] "tas_06"     "tas_07"     "tas_08"     "tas_09"    
 [13] "tas_10"     "tas_11"     "tas_12"     "tmin_01"   
 [17] "tmin_02"    "tmin_03"    "tmin_04"    "tmin_05"   
 [21] "tmin_06"    "tmin_07"    "tmin_08"    "tmin_09"   
 [25] "tmin_10"    "tmin_11"    "tmin_12"    "tmax_01"   
 [29] "tmax_02"    "tmax_03"    "tmax_04"    "tmax_05"   
 [33] "tmax_06"    "tmax_07"    "tmax_08"    "tmax_09"   
 [37] "tmax_10"    "tmax_11"    "tmax_12"    "pr"        
 [41] "pr_01"      "pr_02"      "pr_03"      "pr_04"     
 [45] "pr_05"      "pr_06"      "pr_07"      "pr_08"     
 [49] "pr_09"      "pr_10"      "pr_11"      "pr_12"     
 [53] "srad_01"    "srad_02"    "srad_03"    "srad_04"   
 [57] "srad_05"    "srad_06"    "srad_07"    "srad_08"   
 [61] "srad_09"    "srad_10"    "srad_11"    "srad_12"   
 [65] "sfcWind_01" "sfcWind_02" "sfcWind_03" "sfcWind_04"
 [69] "sfcWind_05" "sfcWind_06" "sfcWind_07" "sfcWind_08"
 [73] "sfcWind_09" "sfcWind_10" "sfcWind_11" "sfcWind_12"
 [77] "vapr_01"    "vapr_02"    "vapr_03"    "vapr_04"   
 [81] "vapr_05"    "vapr_06"    "vapr_07"    "vapr_08"   
 [85] "vapr_09"    "vapr_10"    "vapr_11"    "vapr_12"   
 [89] "model"      "period"     "source"     "lm"        
 [93] "Continent"  "Type"       "Name"       "Acronym"   
 [97] "z"          "tmin"       "tmax"       "vapr"      
[101] "srad"       "sfcWind"   

```{r aridity-index-tho, eval = F}
wdc.tho <- read.table("Worldclim/wdc.txt")

wdc.tho <- wdc.tho %>% mutate(
                 i_01 = (tas_01/5)^1.514,
                 i_02 = (tas_02/5)^1.514,
                 i_03 = (tas_03/5)^1.514,
                 i_04 = (tas_04/5)^1.514,
        ...
        ET0_01 = 16* geosphere::daylength(lat,"2000-01-15")/12* 31 /30*(10*tas_01/I)^a, #Ep = 16 * L/12 * N/30 * (10T/I) ^a, L is length of daylight, N number of days in month
        ET0_02 = 16* geosphere::daylength(lat,"2000-02-15")/12* 28 /30*(10*tas_02/I)^a,
        ET0_03 = 16* geosphere::daylength(lat,"2000-03-15")/12* 31 /30*(10*tas_03/I)^a,
        ET0_04 = 16* geosphere::daylength(lat,"2000-04-15")/12* 30 /30*(10*tas_04/I)^a,
        ET0_05 = 16* geosphere::daylength(lat,"2000-05-15")/12* 31 /30*(10*tas_05/I)^a,
        ET0_06 = 16* geosphere::daylength(lat,"2000-06-15")/12* 30 /30*(10*tas_06/I)^a,
        ET0_07 = 16* geosphere::daylength(lat,"2000-07-15")/12* 31 /30*(10*tas_07/I)^a,
        ET0_08 = 16* geosphere::daylength(lat,"2000-08-15")/12* 31 /30*(10*tas_08/I)^a,
        ET0_09 = 16* geosphere::daylength(lat,"2000-09-15")/12* 30 /30*(10*tas_09/I)^a,
        ET0_10 = 16* geosphere::daylength(lat,"2000-10-15")/12* 31 /30*(10*tas_10/I)^a,
        ET0_11 = 16* geosphere::daylength(lat,"2000-11-15")/12* 30 /30*(10*tas_11/I)^a,
        ET0_12 = 16* geosphere::daylength(lat,"2000-12-15")/12* 31 /30*(10*tas_12/I)^a,
        ET0 = ET0_01+ET0_02+ET0_03+ET0_04+ET0_05+ET0_06+ET0_07+ET0_08+ET0_09+ET0_10+ET0_11,ET0_12, # en mm
        pr = pr_01 + pr_02 + pr_03 + pr_04 + pr_05 + pr_06 + pr_07 + pr_08 + pr_09 + pr_10 + pr_11 + pr_12, # in worldclim the monthly precipitation are already the sum of the mean daily precipitation  
        AI = pr/ET0)
...
wdc.tho$cat.AI <- wdc.tho$AI %>% cut(breaks = breaks.unesco) %>% 
  revalue(cat.unesco)
wdc.tho$cat.AI[which(wdc.tho$ET0 < 400)] <- "Cold"

write.table(wdc.tho, "Worldclim/wdc.tho.txt")
```

```{r plot-maps-tho}
wdc.tho <- read.table("Worldclim/wdc.tho.txt")

ggplot() + geom_tile(data = wdc.tho, aes(x=lon, y = lat,  fill = ET0))+
  scale_fill_binned(type = "viridis", breaks = c(0, 400, 800, 1200, 1600), na.value = "gray90")+
  theme_void()+
  theme(legend.position = "bottom")

ggplot() + geom_raster(data = subset(wdc.tho, lm == 1), aes(x=lon, y = lat,  fill = cat.AI))+
  geom_raster(data = subset(wdc.tho, lm != 1), aes(x=lon, y = lat),  fill = "lightblue")+
  scale_fill_manual(values = col.cat)+
  labs(fill = "")+
  ylim(-55,90)+
  theme_void()+
...
```{r aridity-index-har eval = F}
wdc.har <- read.table("Worldclim/wdc.txt")


  
wdc.har$tmax <- apply(dplyr::select(wdc.har, c("tmax_01", "tmax_02", "tmax_03", "tmax_04", "tmax_05", "tmax_06", "tmax_07", "tmax_08", "tmax_09", "tmax_10", "tmax_11", "tmax_12")), 1, max, na.rm = T)
wdc.har$tmin <- apply(dplyr::select(wdc.har, c("tmin_01", "tmin_02", "tmin_03", "tmin_04", "tmin_05", "tmin_06", "tmin_07", "tmin_08", "tmin_09", "tmin_10", "tmin_11", "tmin_12")), 1, min, na.rm = T)

wdc.har <- wdc.har %>% 
  mutate(psi = pi/180*lat, # latitude in radian
  delta_01 = 0.409 * sin(2*pi/365*yday("1985-01-15")-1.39), #solar declination
  delta_02 = 0.409 * sin(2*pi/365*yday("1985-02-15")-1.39), #solar declination
  delta_03 = 0.409 * sin(2*pi/365*yday("1985-03-15")-1.39), #solar declination
  delta_04 = 0.409 * sin(2*pi/365*yday("1985-04-15")-1.39), #solar declination
  delta_05 = 0.409 * sin(2*pi/365*yday("1985-05-15")-1.39), #solar declination
  delta_06 = 0.409 * sin(2*pi/365*yday("1985-06-15")-1.39), #solar declination
  delta_07 = 0.409 * sin(2*pi/365*yday("1985-07-15")-1.39), #solar declination
  delta_08 = 0.409 * sin(2*pi/365*yday("1985-08-15")-1.39), #solar declination
  delta_09 = 0.409 * sin(2*pi/365*yday("1985-09-15")-1.39), #solar declination
  delta_10 = 0.409 * sin(2*pi/365*yday("1985-10-15")-1.39), #solar declination
  delta_11 = 0.409 * sin(2*pi/365*yday("1985-11-15")-1.39), #solar declination
  delta_12 = 0.409 * sin(2*pi/365*yday("1985-12-15")-1.39), #solar declination
  ws_01 = acos(-tan(psi)*tan(delta_01)), #sunset hour angle
  ws_02 = acos(-tan(psi)*tan(delta_02)), #sunset hour angle
  ws_03 = acos(-tan(psi)*tan(delta_03)), #sunset hour angle
  ws_04 = acos(-tan(psi)*tan(delta_04)), #sunset hour angle
  ...
  e_01 = 0.00023*0.408*Ra_01*sqrt(tmax_01-tmin_01)*(tas_01+17.8), #ET for January
  e_02 = 0.00023*0.408*Ra_02*sqrt(tmax_02-tmin_02)*(tas_02+17.8),
  e_03 = 0.00023*0.408*Ra_03*sqrt(tmax_03-tmin_03)*(tas_03+17.8),
  e_04 = 0.00023*0.408*Ra_04*sqrt(tmax_04-tmin_04)*(tas_04+17.8),
  e_05 = 0.00023*0.408*Ra_05*sqrt(tmax_05-tmin_05)*(tas_05+17.8), 
  e_06 = 0.00023*0.408*Ra_06*sqrt(tmax_06-tmin_06)*(tas_06+17.8), 
  e_07 = 0.00023*0.408*Ra_07*sqrt(tmax_07-tmin_07)*(tas_07+17.8), 
  e_08 = 0.00023*0.408*Ra_08*sqrt(tmax_08-tmin_08)*(tas_08+17.8), 
  e_09 = 0.00023*0.408*Ra_09*sqrt(tmax_09-tmin_09)*(tas_09+17.8), 
  e_10 = 0.00023*0.408*Ra_10*sqrt(tmax_10-tmin_10)*(tas_10+17.8), 
  e_11 = 0.00023*0.408*Ra_11*sqrt(tmax_11-tmin_11)*(tas_11+17.8), 
  e_12 = 0.00023*0.408*Ra_12*sqrt(tmax_12-tmin_12)*(tas_12+17.8),
  MAI_01 = pr_01/e_01, #monthly moisture availability index
  MAI_02 = pr_02/e_02,
  MAI_03 = pr_03/e_03,
  MAI_04 = pr_04/e_04,
  MAI_05 = pr_05/e_05,
  ...
  MAI_11 = pr_11/e_11,
  MAI_12 = pr_12/e_12,
  ET0 = rowSums(across(starts_with("e_")), na.rm = T)*365, # en mm/daye_01/p_01
  AI = pr/ET0
  )
...
wdc.har$cat.AI <- wdc.har$AI %>% cut(breaks = breaks.unesco) %>% 
  revalue(cat.unesco)
wdc.har$cat.AI[which(wdc.har$ET0 < 400)] <- "Cold"                    
                    
write.table(wdc.har, "Worldclim/wdc.har.txt")
```

```{r plot-maps-har}
wdc.har <- read.table("Worldclim/wdc.har.txt")

ggplot() + geom_tile(data = wdc.har, aes(x=lon, y = lat,  fill = ET0))+
  scale_fill_binned(type = "viridis", breaks = c(0, 400, 800, 1200, 1600), na.value = "gray90")+
  theme_void()+
  theme(legend.position = "bottom")

ggplot() + geom_raster(data = subset(wdc.har, lm == 1), aes(x=lon, y = lat,  fill = cat.AI))+
  geom_raster(data = subset(wdc.tho, lm != 1), aes(x=lon, y = lat),  fill = "lightblue")+
  scale_fill_manual(values = col.cat)+
  labs(fill = "")+
  ylim(-55,90)+
...
# Compare PM and Hargreaves

```{r map-of-bias-pm-hargreaves}

wdc_harpm <- select(wdc, c("lon","lat","lm","tas","pr","ET0","AI","cat.AI")) %>%
       merge(select(wdc.har,
                    c("lon","lat","lm","ET0","AI","cat.AI")),
             by = c("lon","lat","lm")) %>%
      filter(lm == 1)

wdc_harpm$rb.ET0 <- with(wdc_harpm, (ET0.x-ET0.y)/ET0.x*100)

ggplot(filter(wdc_harpm, rb.ET0 < 100 & rb.ET0 > -100)) + geom_tile(aes(x=lon, y = lat,  fill = rb.ET0))+
  scale_fill_binned_diverging(mid = 0)+
  labs(fill = "Relative biais in ET0 for Hargreaes compared to Penman-Monteith")+
  theme_void()+
  theme(legend.position = "bottom")
 
ggplot() + geom_raster(data = subset(wdc_harpm, ET0.x < ET0.y), aes(x=lon, y = lat,  fill = "PM < H"))+
  geom_raster(data = subset(wdc_harpm, ET0.x > ET0.y), aes(x=lon, y = lat,  fill = "PM > H"))+
  theme_void()+
  theme(legend.position = "bottom")
```
