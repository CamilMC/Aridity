...
#                     
# wdc$Ra <- wdc %>% select(c("Ra_01", "Ra_02", "Ra_02", "Ra_03", "Ra_04", "Ra_05", "Ra_06", "Ra_07", "Ra_08", "Ra_09", "Ra_10", "Ra_11", "Ra_12")) %>% rowMeans()
# wdc$Rso <- with(wdc,Ra*(0.75+2e-5*z))
# wdc$Rnl <- with(wdc, 4.903e-9*((tmax+273.15)^4+(tmin*273.15)^4)/2*(0.34-0.14*sqrt(ea))*(1.35*(srad*1e-3)/Rso - 0.35))
# wdc$Rns <- with(wdc, srad*1e-3 * (1 - 0.23)) # conversion from kJ/m2/day to MJ/m2/day; 0.23 is albedo
# wdc$Rn <- with(wdc,Rns - Rnl) # in MJ/m2/day
...
ggplot() + geom_raster(data = wdc, aes(x=lon, y = lat,  fill = AI))+
  #scale_fill_manual(values = col.cat)+
  theme_void()+
  theme(legend.position = "bottom")
```