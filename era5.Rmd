...
# Comparison hourly and monthly mean for ET
...
```{r ET-by-month-1970}
et0 <- mon70 %>% filter(lm != 0) %>% mutate(Rn = - (msshf + mslhf) * 86400 * 1e-6, #In W/m2, converted to MJ/m2/day
                        P = 101.3 * ((293-0.0065*z)/293)^(5.26),
                        ea = 0.61078*exp(17.27*(tdew-273.15)/((tdew-273.15)+237.3)), # Tetens 1930
...
ggplot(et0_all)+geom_point(aes(x = ET0.x, y = ET0.y, col = cat.AI.x))+scale_color_manual(values = col.cat) + theme_minimal()
...
mpr <- raster("ERA5/pr_compiler.nc", varname = "mtpr", band = i) %>%
  raster::rotate() %>% projectRaster(land_mask) %>%
  as.data.frame(xy = T) %>% setNames(c("lon","lat","mpr")) %>%
  mutate(month = i)
...
tdew <- raster("ERA5/d2m_compiler.nc", varname = "d2m", band = i)  %>% 
  raster::rotate() %>%
  aggregate(fact = 4, fun = mean) %>%
  projectRaster(land_mask) %>%
  as.data.frame(xy = T) %>% setNames(c("lon","lat","tdew")) %>%
...
wind <- raster("ERA5/wind_compiler.nc", varname = "si10", band = i) %>%
  raster::rotate() %>%
  aggregate(fact = 4, fun = mean) %>%
  projectRaster(land_mask) %>%
  as.data.frame(xy = T) %>% 
...
mslhf <- raster("ERA5/mslhf_compiler.nc", varname = "mslhf", band = i) %>%
  raster::rotate() %>%
  aggregate(fact = 4, fun = mean) %>% 
  projectRaster(land_mask) %>% 
  as.data.frame(xy = T) %>% setNames(c("lon","lat","mslhf")) %>%
...
msshf <- raster("ERA5/msshf_compiler.nc", varname = "msshf", band = i) %>%
  raster::rotate() %>% 
  aggregate(fact = 4, fun = mean) %>%
  projectRaster(land_mask) %>%
  as.data.frame(xy = T) %>% setNames(c("lon","lat","msshf"))  %>%
...
jan1985 <- merge(mprm, t2mm, by = c("lon","lat","month")) %>% merge(tdewm, by = c("lon","lat","month")) %>% merge(windm, by = c("lon","lat","month")) %>% 
  merge(mslhfm, by = c("lon","lat","month")) %>% merge(msshfm, by = c("lon","lat","month")) %>% 
  merge(land_mask.df, by = c("lon","lat")) %>%
  merge(ipcc_regions.df, by = c("lon","lat")) %>%
  merge(elev.df, by = c("lon","lat"))
...
```{r jan1985-ref}
jan85_day <- jan1985_day %>% filter(lm != 0) %>% mutate(Rn = - (msshf + mslhf) * 86400 * 1e-6, #In W/m2, converted to MJ/m2/day
                        P = 101.3 * ((293-0.0065*z)/293)^(5.26),
                        ea = 0.61078*exp(17.27*(tdew-273.15)/((tdew-273.15)+237.3)), # Tetens 1930
                        es = 0.61078*exp(17.27*(t2m-273.15)/((t2m-273.15)+237.3)),
                        delta = 4098*es/((t2m-273.15)+237.3)^2,
...
```{r jan1985-avg}
jan85_avg <- jan1985_day %>% filter(lm != 0) %>% group_by(lon,lat,lm, Continent, Type, Name, Acronym, z) %>% 
                summarise(mpr = mean(mpr, na.rm = T), t2m = mean(t2m, na.rm = T), tdew = mean(tdew, na.rm = T), 
                          wind = mean(wind, na.rm = T), msshf = mean(msshf, na.rm = T), mslhf = mean(mslhf, na.rm = T)) %>%
                 mutate(Rn = - (msshf + mslhf) * 86400 * 1e-6, #In W/m2, converted to MJ/m2/day
                        P = 101.3 * ((293-0.0065*z)/293)^(5.26),
...
jan85_avg$cat.AI <- jan85_day$AI %>% cut(breaks = breaks.unesco) %>%  revalue(cat.unesco)
...
ggplot(jan85_all)+geom_point(aes(x = ET0.x, y = ET0.y, col = cat.AI.x))+scale_color_manual(values = col.cat) + theme_minimal()

ggplot(jan85_all) + geom_raster(aes(x=lon, y = lat,  fill = cat.AI.x))+
  scale_fill_manual(values = col.cat)+
  theme_void()+
  theme(legend.position = "bottom")

ggplot(jan85_all) + geom_raster(aes(x=lon, y = lat,  fill = cat.AI.y))+
  scale_fill_manual(values = col.cat)+
  theme_void()+
  theme(legend.position = "bottom")
```
