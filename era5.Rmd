...
Names in https://confluence.ecmwf.int/display/CKB/ERA5%3A+data+documentation
...
mpr <- raster("ERA5/ERA5_data/mean_rates.nc", varname = "mtpr") %>%
  stack() %>% 
  aggregate(fact = 4, fun = mean) %>%
...
write.table(mpr.df, "ERA5/ERA5_data/mpr.df.txt")



tas <- raster("ERA5/ERA5_data/temperature-2m.nc") %>%
  stack() %>%
  aggregate(fact = 4, fun = mean) %>%
  raster::rotate() %>%
  projectRaster(land_mask) %>%
...
write.table(tas.df, "ERA5/ERA5_data/tas.df.txt")

tdew <- raster("ERA5/ERA5_data/2m-dewpoint.nc", varname = "d2m")  %>% 
  stack() %>% 
  aggregate(fact = 4, fun = mean) %>%
  raster::rotate() %>%
  projectRaster(land_mask) %>% 
...
write.table(tdew.df, "ERA5/ERA5_data/tdew.df.txt")

# ssrd <- raster("ERA5/ERA5_data/surface-short-wave-solar-radiation-downwards.nc") %>%
#   stack() %>%
#   aggregate(fact = 4, fun = mean) %>%
#   raster::rotate() %>%
#   projectRaster(land_mask) %>%
...
# write.table(ssrd.df, "ERA5/ERA5_data/ssrd.df.txt")

wind <- raster("ERA5/ERA5_data/wind10m.nc", varname = "si10") %>%
  stack() %>%
  aggregate(fact = 4, fun = mean) %>%
  raster::rotate() %>%
  projectRaster(land_mask) %>%
...
write.table(wind.df, "ERA5/ERA5_data/wind.df.txt")

# Evaporation in m of water equivalent
# evaporation <- raster("ERA5/ERA5_data/evaporation.nc") %>% stack() %>%  raster::stackApply(indices = 1, fun = mean, na.rm = T) %>%  raster::rotate() %>% aggregate(fact = 4, fun = mean) %>% projectRaster(land_mask)
# evaporation.df <- as.data.frame(evaporation, xy = T) %>% setNames(c("lon","lat","evap"))
# write.table(evaporation.df, "ERA5/ERA5_data/evaporation.df.txt")

# mevap <- raster("ERA5/ERA5_data/mean_rates.nc", varname = "mer") %>% stack() %>%  raster::stackApply(indices = 1, fun = mean, na.rm = T) %>%  raster::rotate() %>% aggregate(fact = 4, fun = mean) %>% projectRaster(land_mask)
# mevap.df <- as.data.frame(mevap, xy = T) %>% setNames(c("lon","lat","mevap"))
# write.table(mevap.df, "ERA5/ERA5_data/mevap.df.txt")

# mpevap <- raster("ERA5/ERA5_data/mean-potential-evaporation-rate.nc") %>% stack() %>%  raster::stackApply(indices = 1, fun = mean, na.rm = T) %>%  raster::rotate() %>% aggregate(fact = 4, fun = mean) %>% projectRaster(land_mask)
# mpevap.df <- as.data.frame(mpevap, xy = T) %>% setNames(c("lon","lat","mpevap"))
# write.table(mpevap.df, "ERA5/ERA5_data/mpevap.df.txt")

# pot.evaporation <- raster("ERA5/ERA5_data/potential-evaporation.nc") %>% stack() %>%  raster::stackApply(indices = 1, fun = mean, na.rm = T) %>%  raster::rotate() %>% aggregate(fact = 4, fun = mean) %>% projectRaster(land_mask)
# pevap.df <- as.data.frame(pot.evaporation, xy = T) %>% setNames(c("lon","lat","pevap"))
# write.table(pevap.df, "ERA5/ERA5_data/pevap.df.txt")

# slhf <- raster("ERA5/ERA5_data/surface_latent_heat_flux.nc", varname = "slhf") %>% stack() %>%  raster::stackApply(indices = 1, fun = mean, na.rm = T) %>%  raster::rotate() %>% aggregate(fact = 4, fun = mean) %>% projectRaster(land_mask)
# slhf.df <- as.data.frame(slhf, xy = T) %>% setNames(c("lon","lat","slhf"))
# write.table(slhf.df, "ERA5/slhf.df.txt")
# 
# 
# sshf <- raster("ERA5/ERA5_data/surface_sensible_heat_flux.nc", varname = "sshf") %>% stack() %>%  raster::stackApply(indices = 1, fun = mean, na.rm = T) %>%  raster::rotate() %>% aggregate(fact = 4, fun = mean) %>% projectRaster(land_mask)
# sshf.df <- as.data.frame(slhf, xy = T) %>% setNames(c("lon","lat","sshf"))
# write.table(sshf.df, "ERA5/ERA5_data/sshf.df.txt")

mslhf <- raster("ERA5/ERA5_data/mean_rates.nc", varname = "mslhf") %>%
  stack() %>%
  aggregate(fact = 4, fun = mean) %>% 
  raster::rotate() %>%
  projectRaster(land_mask) %>%
...
write.table(mslhf.df, "ERA5/ERA5_data/mslhf.df.txt")


msshf <- raster("ERA5/ERA5_data/mean_rates.nc", varname = "msshf") %>%
  stack() %>%
  aggregate(fact = 4, fun = mean) %>%
  raster::rotate() %>% 
  projectRaster(land_mask) %>%
...
write.table(msshf.df, "ERA5/ERA5_data/msshf.df.txt")

msr <- raster("ERA5/ERA5_data/mean-net-SW-LW-radiation-flux.nc", varname = "msnswrf") %>%
  stack() %>% 
  aggregate(fact = 4, fun = mean) %>% 
  raster::rotate() %>% 
  projectRaster(land_mask) %>%
...
write.table(msr.df, "ERA5/ERA5_data/msr.df.txt")


mlr <- raster("ERA5/ERA5_data/mean-net-SW-LW-radiation-flux.nc", varname = "msnlwrf") %>%
  stack() %>% 
  aggregate(fact = 4, fun = mean) %>%
  raster::rotate() %>% 
  projectRaster(land_mask) %>%
...
write.table(mlr.df, "ERA5/ERA5_data/mlr.df.txt")
```

```{r make-tas-min, eval = F}
tas_jan <- raster("ERA5/ERA5_data/temperature-2m.nc") %>% stack(bands = seq(from = 1, to = 349, by = 12)) %>%  raster::stackApply(indices = 1, fun = mean, na.rm = T) %>%  raster::rotate() %>% projectRaster(land_mask)

tas_jan.df <- as.data.frame(tas_jan, xy = T) %>% setNames(c("lon","lat","tas_01"))

tas_fev <- raster("ERA5/ERA5_data/temperature-2m.nc") %>% stack(bands = seq(from = 2, to = 350, by = 12)) %>%  raster::stackApply(indices = 1, fun = mean, na.rm = T) %>%  raster::rotate() %>% projectRaster(land_mask)

tas_fev.df <- as.data.frame(tas_fev, xy = T) %>% setNames(c("lon","lat","tas_02"))

tas_mar <- raster("ERA5/ERA5_data/temperature-2m.nc") %>% stack(bands = seq(from = 3, to = 351, by = 12)) %>%  raster::stackApply(indices = 1, fun = mean, na.rm = T) %>%  raster::rotate() %>% projectRaster(land_mask)

tas_mar.df <- as.data.frame(tas_mar, xy = T) %>% setNames(c("lon","lat","tas_03"))

tas_apr <- raster("ERA5/ERA5_data/temperature-2m.nc") %>% stack(bands = seq(from = 4, to = 352, by = 12)) %>%  raster::stackApply(indices = 1, fun = mean, na.rm = T) %>%  raster::rotate() %>% projectRaster(land_mask)

tas_apr.df <- as.data.frame(tas_apr, xy = T) %>% setNames(c("lon","lat","tas_04"))

tas_may <- raster("ERA5/ERA5_data/temperature-2m.nc") %>% stack(bands = seq(from = 5, to = 353, by = 12)) %>%  raster::stackApply(indices = 1, fun = mean, na.rm = T) %>%  raster::rotate() %>% projectRaster(land_mask)

tas_may.df <- as.data.frame(tas_may, xy = T) %>% setNames(c("lon","lat","tas_05"))

tas_jun <- raster("ERA5/ERA5_data/temperature-2m.nc") %>% stack(bands = seq(from = 6, to = 354, by = 12)) %>%  raster::stackApply(indices = 1, fun = mean, na.rm = T) %>%  raster::rotate() %>% projectRaster(land_mask)

tas_jun.df <- as.data.frame(tas_jun, xy = T) %>% setNames(c("lon","lat","tas_06"))

tas_jul <- raster("ERA5/ERA5_data/temperature-2m.nc") %>% stack(bands = seq(from = 7, to = 355, by = 12)) %>%  raster::stackApply(indices = 1, fun = mean, na.rm = T) %>%  raster::rotate() %>% projectRaster(land_mask)

tas_jul.df <- as.data.frame(tas_jul, xy = T) %>% setNames(c("lon","lat","tas_07"))

tas_aug <- raster("ERA5/ERA5_data/temperature-2m.nc") %>% stack(bands = seq(from = 8, to = 356, by = 12)) %>%  raster::stackApply(indices = 1, fun = mean, na.rm = T) %>%  raster::rotate() %>% projectRaster(land_mask)

tas_aug.df <- as.data.frame(tas_aug, xy = T) %>% setNames(c("lon","lat","tas_08"))

tas_sep <- raster("ERA5/ERA5_data/temperature-2m.nc") %>% stack(bands = seq(from = 9, to = 357, by = 12)) %>%  raster::stackApply(indices = 1, fun = mean, na.rm = T) %>%  raster::rotate() %>% projectRaster(land_mask)

tas_sep.df <- as.data.frame(tas_sep, xy = T) %>% setNames(c("lon","lat","tas_09"))

tas_oct <- raster("ERA5/ERA5_data/temperature-2m.nc") %>% stack(bands = seq(from = 10, to = 358, by = 12)) %>%  raster::stackApply(indices = 1, fun = mean, na.rm = T) %>%  raster::rotate() %>% projectRaster(land_mask)

tas_oct.df <- as.data.frame(tas_oct, xy = T) %>% setNames(c("lon","lat","tas_10"))

tas_nov <- raster("ERA5/ERA5_data/temperature-2m.nc") %>% stack(bands = seq(from = 11, to = 359, by = 12)) %>%  raster::stackApply(indices = 1, fun = mean, na.rm = T) %>%  raster::rotate() %>% projectRaster(land_mask)

tas_nov.df <- as.data.frame(tas_nov, xy = T) %>% setNames(c("lon","lat","tas_11"))

tas_dec <- raster("ERA5/ERA5_data/temperature-2m.nc") %>% stack(bands = seq(from = 12, to = 3360, by = 12)) %>%  raster::stackApply(indices = 1, fun = mean, na.rm = T) %>%  raster::rotate() %>% projectRaster(land_mask)
...
write.table(tas_month, "ERA5/ERA5_data/tas_month.txt")
```

```{r merge-dfs, eval = T}
# pr.df <- read.table("ERA5/ERA5_data/pr.df.txt")
mpr.df <- read.table("ERA5/ERA5_data/mpr.df.txt")
tas.df <- read.table("ERA5/ERA5_data/tas.df.txt")
#tas_month.df <- read.table("ERA5/ERA5_data/tas_month.txt")
tdew.df <- read.table("ERA5/ERA5_data/tdew.df.txt")
ssrd.df <- read.table("ERA5/ERA5_data/ssrd.df.txt")
wind.df <- read.table("ERA5/ERA5_data/wind.df.txt")
...
mslhf.df <- read.table("ERA5/ERA5_data/mslhf.df.txt")
msshf.df <- read.table("ERA5/ERA5_data/msshf.df.txt")
msr.df <- read.table("ERA5/ERA5_data/msr.df.txt")
mlr.df <- read.table("ERA5ERA5_data//mlr.df.txt")
...
## AI = average P / average PET

the variable in era5 are averaged over the 30 years 
...
ET0 calculated from the 30 years averaged variables (AI = mean(P/ET0)), named "ref"
...
ET0 calculated from the 30 years averaged variables (AI = mean(P)/mean(ET0)), named "_30y"
...
Based on year 1985, we compare the AI calculated 1) as the mean of the daily AI : AI = mean (P/ET0), in jan85_ref in which ET0 is calculated using all individual values for each variable (no averaging of variables before calculating ET0); and 2) as the mean precipitation divided by the mean ET0: AI = mean(P)/mean(ET0) in which ET0 is calculated from the mean values of each variable, in jan85_avg.  (by pixel, for 1 month)
...
mpr <- raster("ERA5/january1985/pr_compiler.nc", band = i) %>%
  raster::rotate() %>% projectRaster(land_mask) %>%
  as.data.frame(xy = T) %>% setNames(c("lon","lat","mpr")) %>%
  mutate(month = i)
...
t2m <- raster("ERA5/january1985/t2m_compiler.nc", band = i) %>%
  raster::rotate() %>%
  aggregate(fact = 4, fun = mean) %>%
  projectRaster(land_mask) %>%
  as.data.frame(xy = T) %>% setNames(c("lon","lat","t2m")) %>%
...
tdew <- raster("ERA5/january1985/d2m_compiler.nc", band = i)  %>% 
  raster::rotate() %>%
  aggregate(fact = 4, fun = mean) %>%
  projectRaster(land_mask) %>%
  as.data.frame(xy = T) %>% setNames(c("lon","lat","tdew")) %>%
...
uwind <- raster("ERA5/january1985/10m-u-wind_compiler.nc", band = i) %>%
  raster::rotate() %>%
  aggregate(fact = 4, fun = mean) %>%
  projectRaster(land_mask) %>%
  as.data.frame(xy = T) %>% 
...
vwind <- raster("ERA5/january1985/10m-v-wind_compiler.nc", band = i) %>%
  raster::rotate() %>%
  aggregate(fact = 4, fun = mean) %>%
  projectRaster(land_mask) %>%
  as.data.frame(xy = T) %>% 
...
mslhf <- raster("ERA5/january1985/mslhf_compiler.nc", band = i) %>%
  raster::rotate() %>%
  aggregate(fact = 4, fun = mean) %>% 
  projectRaster(land_mask) %>% 
  as.data.frame(xy = T) %>% setNames(c("lon","lat","mslhf")) %>%
...
msshf <- raster("ERA5/january1985/msshf_compiler.nc", band = i) %>%
  raster::rotate() %>% 
  aggregate(fact = 4, fun = mean) %>%
  projectRaster(land_mask) %>%
  as.data.frame(xy = T) %>% setNames(c("lon","lat","msshf"))  %>%
...
write.table(jan1985, "ERA5/january1985/jan1985.txt")
```

```{r jan1985-ref}
jan1985 <- read.table("ERA5/january1985/jan1985.txt")
...
write.table(jan85_ref, "ERA5/january1985/jan85_ref.txt")
```
...
write.table(jan85_avg, "ERA5/january1985/jan85_avg.txt")
```


```{r compare-jan1985}
jan85_ref <- read.table("ERA5/january1985/jan85_ref.txt")
jan85_avg <- read.table("ERA5/january1985/jan85_avg.txt")
colorvec <- brewer.pal(n = 11, name ="RdYlBu")
...
ggplot(jan85_all) + geom_tile(aes(x=lon, y = lat,  fill = cat.AI.x))+
  scale_fill_manual(values = col.cat)+
  labs(fill = "AI as mean(P)/mean(ET0)")+
  theme_void()+
  theme(legend.position = "bottom")

ggplot(jan85_all) + geom_tile(aes(x=lon, y = lat,  fill = cat.AI.y))+
  scale_fill_manual(values = col.cat)+
    labs(fill = "AI as mean(P/ET0)")+
  theme_void()+
  theme(legend.position = "bottom")
...
ggplot(data = jan85_all, aes(x = ET0.x, y = ET0.y))+geom_point(aes(col = cat.AI.x))+
     ...
     geom_abline(slope = 1, intercept = 0)+
     labs(x = "ET0 as mean(ET0_i)", y = "ET0 as ET0 = f(mean(T), mean(Rn), mean(sfcWind), etc)", title = "ET0 for january 1985", col = "")+
...
ggplot(subset(jan85_all, lm ==1)) + geom_tile(aes(x=lon, y = lat,  fill = ET0.y - ET0.x))+
  ...
  labs(title = "difference between ET0=f(mean(T,Wind,Rn...)) and mean(ET0 = f(T,Wind, Rn ...))")+
  theme_void()+
  theme(legend.position = "bottom")
...
ggplot(subset(jan85_all, lat < 50 & lat > -50), aes(x = AI.x, y = AI.y))+geom_point(aes(col = lat), alpha = 0.5, shape = 1)+
  ...
  labs(title = "comparison of AI as mean(P)/mean(ET0) and AI as mean(P/ET0)", x = "AI as mean(P/ET0)", y = "AI as mean(P)/mean(ET)")+ theme_minimal()
```

## March 2023

Same comparison but for March 2023
...
mar23_ref: AI = mean(P/ET0) with ET0 = f(T,wind,Rn, ...)

mar23_avg: AI = mean(P)/mean(ET0) with ET0 = f(mean(T),mean(wind),mean(Rn), ...)
...
write.table(mar23_ref, "ERA5/march2023/mar23_ref.txt")
```
...
# SPEI 1970_2000
```{r try-spi-era5}
spi_test <- mon1970_2000 %>% filter(lon == 0.5, lat == 0.5) 
spi_test <- spi_test[order(spi_test$month),]
