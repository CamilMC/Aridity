...
```{r clim-summary}
clim_sum <- clim %>% subset(source != "Wordlclim") %>% group_by(lon, lat, model, period) %>% dplyr::summarise(ts = mean(ts, na.rm = T), pr = mean(pr, na.rm = T), AIm = mean(AIm, na.rm = T), AIu = mean(AIu, na.rm = T))
...
# Divergence CMIP6/Worldclim

```{r divergence-from-wdc}
clim_sum_div <- subset(clim_sum, period == "1970_2000") %>% select(c("lon","lat","AIm","AIu"))
wdc_div <- clim %>% subset(source == "Worldclim") %>% select(c("lon","lat","AIm","AIu"))
div <- merge(clim_sum_div, wdc_div, by = c("lon","lat"))

# AIm
div$div.AIm <- with(div,abs((AIm.x-AIm.y)/AIm.y*100))

ggplot(filter(div, AIm.y < 1000 & AIm.y > 0 & AIm.x < 1000 & AIm.x > 0)) + geom_boxplot(aes(x="CMIP6",y = AIm.x))+
  geom_boxplot(aes(x="WDC",y = AIm.y))

quantile(div$div.AIm, probs = seq (0,1,0.05), na.rm = T)
  
  
ggplot() + geom_raster(data = subset(div, AIm.y > 0 & AIm.y < 200), aes(x=lon, y = lat,  fill = AIm.y))+
  geom_raster(data = subset(div, AIm.y < 0), aes(x=lon, y = lat),  fill = "gray60")+
  geom_raster(data = subset(div, AIm.y > 200), aes(x=lon, y = lat),  fill = "black")+
  scale_fill_viridis_c(direction = -1)+
  theme_void()+
  theme(legend.position = "bottom")

ggplot() + geom_raster(data = subset(div, div.AIm < 287.83), aes(x=lon, y = lat,  fill = div.AIm))+
  geom_raster(data = subset(div, div.AIm > 287.38), aes(x=lon, y = lat),  fill = "black")+
  scale_fill_viridis_c(direction = -1, name = "% difference between CMIP6 and WDC (< 300%)")+
  theme_void()+
  theme(legend.position = "bottom")

div$uncertainty.AIm <- with(div, ifelse(div.AIm < 10, "in","out"))

ggplot() + geom_raster(data = subset(clim_sum, period == "1970_2000" & AIm >= 0), aes(x=lon, y = lat,  fill = cat.AIm))+
  geom_raster(data = subset(clim_sum, period == "1970_2000" & AIm < 0), aes(x=lon, y = lat), fill = "gray60")+
  geom_point(data = filter(div, uncertainty.AIm == "in"), aes(x = lon, y = lat), col = "black", shape = "'")+
  scale_fill_manual(values = col.martonne)+
  theme_void()+
  theme(legend.position = "bottom")

# AIu
div$div.AIu <- with(div,abs((AIu.x-AIu.y)/AIu.y*100))

ggplot(filter(div, AIu.y < 1000 & AIu.y > 0 & AIu.x < 1000 & AIu.x > 0)) + geom_boxplot(aes(x="CMIP6",y = AIu.x))+
  geom_boxplot(aes(x="WDC",y = AIu.y))

quantile(div$div.AIu, probs = seq (0,1,0.05), na.rm = T)
  
  
ggplot() + geom_raster(data = subset(div, AIu.y > 0 & AIu.y < 200), aes(x=lon, y = lat,  fill = AIu.y))+
  geom_raster(data = subset(div, AIu.y < 0), aes(x=lon, y = lat),  fill = "gray60")+
  geom_raster(data = subset(div, AIu.y > 200), aes(x=lon, y = lat),  fill = "black")+
  scale_fill_viridis_c(direction = -1)+
  theme_void()+
  theme(legend.position = "bottom")

ggplot() + geom_raster(data = subset(div, div.AIu < 287.83), aes(x=lon, y = lat,  fill = div.AIu))+
  geom_raster(data = subset(div, div.AIu > 287.38), aes(x=lon, y = lat),  fill = "black")+
  scale_fill_viridis_c(direction = -1, name = "% difference between CMIP6 and WDC (< 300%)")+
  theme_void()+
  theme(legend.position = "bottom")

div$uncertainty.AIu <- with(div, ifelse(div.AIu < 10, "in","out"))

ggplot() + geom_raster(data = subset(clim_sum, period == "1970_2000"), aes(x=lon, y = lat,  fill = cat.AIu))+
  geom_point(data = filter(div, uncertainty.AIu == "in"), aes(x = lon, y = lat), col = "black", shape = "'")+
  scale_fill_manual(values = col.martonne)+
  theme_void()+
  theme(legend.position = "bottom")
