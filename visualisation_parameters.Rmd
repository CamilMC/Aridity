...
wdctt <- wdct %>% projectRaster(to = land_mask) %>% as.data.frame(xy = T) %>% setNames(c("lon","lat","ts"))

wdcp <- raster("Worldclim/wc2.1_10m/wc2.1_10m_bio_12.tif") 
wdcpp <- wdcp %>% projectRaster(to = land_mask) %>% as.data.frame(xy = T) %>% setNames(c("lon","lat","pr"))
...
wdcww <- wdcw %>% projectRaster(to = land_mask) %>% as.data.frame(xy = T) %>% setNames(c("lon","lat","sfcWind"))
...
wdcrr <- wdcr %>% projectRaster(to = land_mask) %>% as.data.frame(xy = T) %>% setNames(c("lon","lat","rsds"))
...
clim_med <- clim %>% group_by(lon, lat, model, period) %>% dplyr::summarise(ts = median(ts, na.rm = T), pr = median(pr, na.rm = T), AIm = median(AIm, na.rm = T), AIu = median(AIu, na.rm = T))
...
g <- ggplot(data = filter(clim, period == "1850_1880" & source == i)) + geom_raster(aes(x=lon, y = lat,  fill = cat.AIu))+
   scale_fill_manual(values = col.cat)+
...
list.table <- list.files(path = "CMIP6/ts/", pattern = "hist.1970-2000.ts.txt", full.names = T)

for(i in list.table){
g <- ggplot(data = read.table(i)) + geom_raster(aes(x=lon, y = lat,  fill = ts-273.15))+
    scale_fill_continuous_divergingx(palette = "RdYlBu", rev = T, mid = 0, limits = c(-60,40))+
    labs(title = i, fill = "")+
    theme_void()+
   theme(legend.position = "bottom")
...
bcc <- read.table("CMIP6/ts/bcc.hist.1850-1880.ts.txt")

ggplot(data = bcc) + geom_raster(aes(x=lon, y = lat,  fill = ts-273.15))+
    scale_fill_continuous_divergingx(palette = "RdYlBu", rev = T, mid = 0, limits = c(-60,40))+
    labs(title = "BCC historical 1850-1880", fill = "")+
    theme_void()+
   theme(legend.position = "bottom")

bcc <- read.table("CMIP6/ts/bcc.hist.1970-2000.ts.txt")

ggplot(data = bcc) + geom_raster(aes(x=lon, y = lat,  fill = ts-273.15))+
    scale_fill_continuous_divergingx(palette = "RdYlBu", rev = T, mid = 0, limits = c(-60,40))+
    labs(title = "BCC historical 1970-2000", fill = "")+
    theme_void()+
   theme(legend.position = "bottom")

bcc <- read.table("CMIP6/ts/bcc.ssp245.2030-2060.ts.txt")

ggplot(data = bcc) + geom_raster(aes(x=lon, y = lat,  fill = ts-273.15))+
    scale_fill_continuous_divergingx(palette = "RdYlBu", rev = T, mid = 0, limits = c(-60,40))+
    labs(title = "BCC SSP245", fill = "")+
    theme_void()+
   theme(legend.position = "bottom")
```
