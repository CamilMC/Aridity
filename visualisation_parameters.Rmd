...
wdc.t<- raster("Worldclim/wc2.1_10m/wc2.1_10m_bio_1.tif") %>% projectRaster(to = cmcc) %>% as.data.frame(xy = T) %>%
  setNames(c("lon","lat","ts"))
...
wdt <- wdc.t %>% merge(land_mask.df, by = c("lon","lat")) %>% merge(ipcc_regions.df, by = c("lon","lat"))
...
```{r compare-worldclim}
ggplot()+geom_boxplot(data = filter(ts.all_annual, period == "1970_2000"), aes(x = source, y = ts-273, col = source))+
  geom_boxplot(data = wdt, aes(x = "Worldclim", y = ts, col ="Worldclim"))+
   scale_color_viridis_d(option = "A", end = 0.8)+
     labs(x = "", y = "Average annual temperature, °C") + theme_minimal()

for(i in c("EAO", "SAO", "SOO",  "EAN",  "WAF", "SAH", "MED", "WCE",  "NEU",  "ARO",  "CAF",  "WSAF", "SEA", "SIO",  "SAS",  "EAS",  "ECA",  "ESB",  "RAR",  "NAU",  "CAU",  "SAU",  "RFE", "NPO",  "EPO",  "EAU",  "SPO",  "NZ", "ESAF", "SEAF", "NEAF", "ARP",  "MDG",  "WCA",  "EEU",  "EIO",  "ARS",  "WSB",  "TIB",  "BOB")){
  g <- ggplot()+
