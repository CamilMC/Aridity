...
```{r aridity-index}
clim$AIm <- with(clim,pr*60*60*24*365/(ts-273.15+10)) # pr in mm/y
...
clim$cat.martonne <- clim$AIm %>% cut(breaks.martonne, include.lowest = T) %>% revalue(cat.martonne)
...
ggplot() + geom_raster(data = subset(clim, period == "1970_2000" & source == "CNRM" & AIm >= 0), aes(x=lon, y = lat,  fill = cat.martonne))+
  geom_raster(data = subset(clim, period == "1970_2000" & source == "CNRM" & AIm < 0), aes(x=lon, y = lat), fill = "gray60")+
