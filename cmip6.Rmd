...
# hfls.all_annual <- read.table("CMIP6/hlfs/hfls.all_annual.txt")
...
#  merge(tasmin.all_annual, by = c("lon","lat","model","period","lm", "Continent","Type","Name","Acronym","source")) %>%
...
cmip6$AIm <- with(cmpi6,pr*60*60*24*365/(tas-273.15+10)) # pr in mm/y, tas in Ceslsius
...
cmip6$cat.AI <- cmip6$AIu %>% cut(breaks = breaks.unesco) %>% 
  revalue(cat.unesco)

ggplot(data = subset(cmip6, period == "1970_2000")) + geom_raster(aes(x=lon, y = lat,  fill = cat.AI))+
