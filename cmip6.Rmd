...
multimodel <- cmip6 %>% subset(period == "1970_2000" & !Continent %in% c("SOUTHERN","PACIFIC","POLAR","ATLANTIC","INDIAN","ARCTIC")) %>%
  group_by(cat.AI) %>%
  summarise(count = n()) %>%
...
tab <- cmip6 %>% subset(period == "1970_2000" & !Continent %in% c("SOUTHERN","PACIFIC","POLAR","ATLANTIC","INDIAN","ARCTIC")) %>%
...
cmip6s$change.cat.maj <- with(cmip6s, ifelse(catmajref == "Arid" & cat.maj %in% c("Cold","Humid"),"Arid to Cold or Humid",
                                      ...
                                      ifelse(catmajref == "Semi-arid" & cat.maj %in% c("Arid", "Hyperarid"),"Semi-arid to arid or hyperarid",
...
for(i in c("1850_1880","1985_2015")){
g <- ggplot() + geom_raster(data = subset(cmip6s, period == i & model == "historical" & change.catmaj == 1 & dryer.maj == "dryer"), aes(x=lon, y = lat,  fill = dryer.maj))+
  borders()+
  scale_fill_manual(values =  c("#F46D43", "#74ADD1"), na.translate = F)+
  labs(title = paste("AI changes between",i , "and 1970-2000", sep = " "), fill = "")+
  theme_void()+ylim(-55,90)+
...
```{r map-future-changes-dry, fig.dim = c(16,8)}
...
tab.future <- cmip6s %>% subset(!Continent %in% c("SOUTHERN","PACIFIC","POLAR","ATLANTIC","INDIAN","ARCTIC")) %>%
...
tab.future.maj <- cmip6s %>% subset(!Continent %in% c("SOUTHERN","PACIFIC","POLAR","ATLANTIC","INDIAN","ARCTIC")) %>%
...
tab.future.cont <- cmip6s %>% subset(!Continent %in% c("SOUTHERN","PACIFIC","POLAR","ATLANTIC","INDIAN","ARCTIC")) %>%
