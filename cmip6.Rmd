...
```{r maps-hist-1850-1880, fig.dim = 15,20}
map_list <- list()
...
for(i in unique(cmip6$source)){
g <- ggplot(data = subset(cmip6, period == "1985_2010" & source == i)) + geom_raster(aes(x=lon, y = lat,  fill = cat.AI))+
  scale_fill_manual(values = col.cat, na.translate = F)+
...
for(i in unique(cmip6$source)){
g <- ggplot(data = subset(cmip6, period == "2030-2060" & model == "SSP245" & source == i)) + geom_raster(aes(x=lon, y = lat,  fill = cat.AI))+
  scale_fill_manual(values = col.cat, na.translate = F)+
...
for(i in unique(cmip6$source)){
g <- ggplot(data = subset(cmip6, period == "2070-2100" & model == "SSP245" & source == i)) + geom_raster(aes(x=lon, y = lat,  fill = cat.AI))+
  scale_fill_manual(values = col.cat, na.translate = F)+
...
for(i in unique(cmip6$source)){
g <- ggplot(data = subset(cmip6, period == "2030-2060" & model == "SSP370" & source == i)) + geom_raster(aes(x=lon, y = lat,  fill = cat.AI))+
  scale_fill_manual(values = col.cat, na.translate = F)+
  labs(title = paste(i, "2030-2060", sep = ", "), fill = "")+
  theme_void()
map_list[[i]] <- g
...
for(i in unique(cmip6$source)){
g <- ggplot(data = subset(cmip6, period == "2070-2100" & model == "SSP370" & source == i)) + geom_raster(aes(x=lon, y = lat,  fill = cat.AI))+
  scale_fill_manual(values = col.cat, na.translate = F)+
  labs(title = paste(i, "2070-2100", sep = ", "), fill = "")+
  theme_void()
map_list[[i]] <- g
...
for(i in unique(cmip6$source)){
g <- ggplot(data = subset(cmip6, period == "2030-2060" & model == "SSP585" & source == i)) + geom_raster(aes(x=lon, y = lat,  fill = cat.AI))+
  scale_fill_manual(values = col.cat, na.translate = F)+
  labs(title = paste(i, "2030-2060", sep = ", "), fill = "")+
  theme_void()
map_list[[i]] <- g
...
for(i in unique(cmip6$source)){
g <- ggplot(data = subset(cmip6, period == "2070-2100" & model == "SSP585" & source == i)) + geom_raster(aes(x=lon, y = lat,  fill = cat.AI))+
  scale_fill_manual(values = col.cat, na.translate = F)+
  labs(title = paste(i, "2070-2100", sep = ", "), fill = "")+
  theme_void()
map_list[[i]] <- g
...
cmip6s <- cmip6 %>% group_by(lon,lat,Continent, Type, Name, Acronym, lm, model, z, period) %>% 
   dplyr::summarise(t = mean(t, na.rm = T), tas.sd = sd(t, na.rm = T), # t in °C
                  ...
                  drysubhumid = sum(cat.AI == "Dry subhumid") ,
                   ...
                   na = sum(is.na(cat.AI)),
                  nb.models = n(),
                  cat.maj = ifelse(is.na(cat.AI), NA,names(which.max(table(cat.AI)))),
...
for(i in 1:nrow(cmip6s)){
  ...
  maji = cmip6s$cat.maj[i]
  
  cmip6s$maj[i] <- ifelse(maji == catAIi, "same", "diff") 
}
...
cmip6s$dryer <- with(cmip6s, ifelse(change == "Arid to Humid", "wetter",
                                ifelse(change == "Arid to Semi-arid", "dryer",
                                ifelse(change == " Arid to Dry subhumid", "wetter", 
                                ifelse(change == "Arid to Hyperarid", "dryer",
                                ifelse(change == "Arid to Cold", "wetter",       
                                ifelse(change == "Cold to Humid", "wetter",
                                ifelse(change == "Cold to Hyperarid", "dryer",
                                ifelse(change == "Dry subhumid to Cold", "wetter",
                                ifelse(change == "Dry subhumid to Humid", "wetter",
                                ifelse(change == "Dry subhumid to Arid", "dryer",
                                ifelse(change == "Dry subhumid to Semi-arid", "dryer",
                                ifelse(change == "Humid to arid", "dryer",
                                ifelse(change == "Humid to Semi-arid", "dryer",
                                ifelse(change == "Humid to Cold","wetter",
                                ifelse(change == "Humid to Dry subhumid","dryer",
                                ifelse(change == "Semi-arid to Dry subhumid", "wetter",
                                ifelse(change == "Semi-arid to Arid", "dryer",
                                ifelse(change == "Semi-arid to Humid", "wetter",
                                ifelse(change == "Semi-arid to Cold", "wetter",
                                ifelse(change == "Hyperarid to Arid", "wetter",
                                ifelse(change == "Hyperarid to Semi-arid", "wetter",
                                ifelse(change == "Hyperarid to Dry subhumid", "wetter",
                                ifelse(change == "Hyperarid to Humid", "wetter",
                                NA))))))))))))))))))))))))
...
```{r map-future-changes, fig.dim = c(20,20)}
