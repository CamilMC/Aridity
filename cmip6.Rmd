...
table(cmip6s$change.cat.maj)
 ...
 ggplot() + geom_raster(data = subset(cmip6s, change.cat.maj == "Dry subhumid to arid, semi-arid or hyperarid"), aes(x=lon, y = lat), fill = "firebrick")+
   borders()+
  labs(title = "test", fill = "")+
  theme_void()+ylim(-55,90)+
  theme(legend.position = "bottom")
...
for(i in c("1850_1880","1985_2015")){
g <- ggplot() + geom_raster(data = subset(cmip6s, period == i & model == "historical" & change.catmaj == 1 & dryer.maj == "dryer" & 
...
ggarrange(plotlist = map_list, ncol = 2, common.legend = F)
...
colors_changes <- c("Hyperarid to arid" = "#AA44AA", 
                    ...
                    "Humid to semi-arid" = "#ffcc78",
                    "Dry subhumid to arid, semi-arid or hyperarid" = "#ffAA22",
...
#   geom_raster(data = subset(cmip6s, period == i & model == "historical" & change.catmaj == 1 & nb.maj > 10), aes(x= lon, y = lat, fill = "'"))+
  borders()+
  scale_fill_discrete(type = colors_changes)+
#  scale_fill_viridis_d()+
  labs(fill = "Compared to\n1970-2000")+
  theme_void()+ylim(-55,90)+
  theme(legend.position = "bottom")
map_list[[index]] <- g
  }}

lapply(map_list, print)
