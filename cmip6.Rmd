...
ggplot(cmip6s)+geom_boxplot(aes(x=period, y = anomalies))
ggplot(subset(cmip6s, anomalies < 1 & anomalies > -1))+geom_boxplot(aes(x=period, y = anomalies))

for(i in unique(cmip6s$period)){
g <- ggplot(data = subset(cmip6s, period == i)) + geom_tile(aes(x=lon, y = lat,  fill = anomalies))+
...
for(i in unique(cmip6s$period)){
g <- ggplot(data = subset(cmip6s, period == i)) + geom_tile(aes(x=lon, y = lat,  fill = change.cat))+
  labs(title = paste("AI changes between",i, "and 1970-2000", sep = " "))+
  theme_void()+
  theme(legend.position = "bottom")
print(g)
