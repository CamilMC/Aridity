...
# pies <- cmip6s %>% subset(period == "1970_2000" & !Continent %in% c("SOUTHERN","PACIFIC","ATLANTIC","INDIAN","ARCTIC") & lat > -55) %>%
#   select(c("cat.AI")) %>%
#   table() %>% as.data.frame() %>%
#   mutate(perc = round(Freq/sum(Freq)*100, 1),
#          lab.y = rev(cumsum(rev(Freq))) - Freq*0.5)
...
#     scale_fill_manual(values = col.cat)+
#     geom_label_repel(aes(x = "", y = lab.y, label = perc), nudge_x = 0.5)+
#     labs(fill = "", title = "Multimodel CMIP6s")+
#     theme_void()+theme(legend.position = "right")
# 
# print(ps)
```
...
sdmm <- cmip6 %>% subset(period == "1970_2000"  & !Continent %in% c("SOUTHERN","PACIFIC","ATLANTIC","INDIAN","ARCTIC") & lat > -55 ) %>%
  ...
  # summarise(AI.mean = mean(AI, na.rm = T), AI.sd = sd(AI, na.rm = T), cat.AI = cut(AI.mean, breaks.unesco), cat.AI = revalue(cat.AI, cat.unesco)) 
  # sdmm$cat.AI[which(sdmm$ET0 < 400)] <- "Cold"
  # sdmm2 <- sdmm %>% group_by(cat.AI) %>% summarise(count = n()) %>% ungroup
  # sdmm2$percent <- with(sdmm2, round(count/sum(count)*100,1))
...
cmip6smonthly.ref <- cmip6smonthly %>% filter(period == "1970_2000") %>% 
  select(c("lon","lat","model","period", "Continent","Name",
           "AI_01_mean","ET0_01_mean",
           "AI_02_mean","ET0_02_mean",
           "AI_03_mean","ET0_03_mean",
           "AI_04_mean","ET0_04_mean",
           "AI_05_mean","ET0_05_mean",
           "AI_06_mean","ET0_06_mean",
           "AI_07_mean","ET0_07_mean",
           "AI_08_mean","ET0_08_mean",
           "AI_09_mean","ET0_09_mean",
           "AI_10_mean","ET0_10_mean",
           "AI_11_mean","ET0_11_mean",
           "AI_12_mean","ET0_12_mean"
           )) %>% 
  setNames(c("lon","lat","model","period","Continent","Name",
          "AI_01_mean.ref","ET0_01_mean.ref",
           "AI_02_mean.ref","ET0_02_mean.ref",
           "AI_03_mean.ref","ET0_03_mean.ref",
           "AI_04_mean.ref","ET0_04_mean.ref",
           "AI_05_mean.ref","ET0_05_mean.ref",
           "AI_06_mean.ref","ET0_06_mean.ref",
           "AI_07_mean.ref","ET0_07_mean.ref",
           "AI_08_mean.ref","ET0_08_mean.ref",
           "AI_09_mean.ref","ET0_09_mean.ref",
           "AI_10_mean.ref","ET0_10_mean.ref",
           "AI_11_mean.ref","ET0_11_mean.ref",
           "AI_12_mean.ref","ET0_12_mean.ref"
           ))

cmip6smonthly <- merge(cmip6smonthly,cmip6smonthly.ref, by = c("lon","lat","model","period", "Continent","Name"))
...
cmip6smm <- cmip6smonthly %>% group_by(model, period, Continent) %>% 
  summarise(AI_01 = mean(AI_01_mean, na.rm = T), AI_01_sd = sd(AI_01_mean, na.rm = T),
            AI_02 = mean(AI_02_mean, na.rm = T), AI_02_sd = sd(AI_02_mean, na.rm = T),
            AI_03 = mean(AI_03_mean, na.rm = T), AI_03_sd = sd(AI_03_mean, na.rm = T),
            AI_04 = mean(AI_04_mean, na.rm = T), AI_04_sd = sd(AI_04_mean, na.rm = T),
            AI_05 = mean(AI_05_mean, na.rm = T), AI_05_sd = sd(AI_05_mean, na.rm = T),
            AI_06 = mean(AI_06_mean, na.rm = T), AI_06_sd = sd(AI_06_mean, na.rm = T),
            AI_07 = mean(AI_07_mean, na.rm = T), AI_07_sd = sd(AI_07_mean, na.rm = T),
            AI_08 = mean(AI_08_mean, na.rm = T), AI_08_sd = sd(AI_08_mean, na.rm = T),
            AI_09 = mean(AI_09_mean, na.rm = T), AI_09_sd = sd(AI_09_mean, na.rm = T),
            AI_10 = mean(AI_10_mean, na.rm = T), AI_10_sd = sd(AI_10_mean, na.rm = T),
            AI_11 = mean(AI_11_mean, na.rm = T), AI_11_sd = sd(AI_11_mean, na.rm = T),
            AI_12 = mean(AI_12_mean, na.rm = T), AI_12_sd = sd(AI_12_mean, na.rm = T)) %>%
 ungroup() %>% as.data.frame()
