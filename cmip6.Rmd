...
for(i in c("SSP245","SSP370","SSP585")){
  for (j in c("2030_2060","2070_2100")){
index <- paste(i, j, sep = " , ")
g <- ggplot() + geom_raster(data = subset(cmip6s, period == j & model == i & anomalies > 0), aes(x=lon, y = lat,  fill = "wetter"))+
  geom_raster(data = subset(cmip6s, period == j & model == i & anomalies < 0), aes(x=lon, y = lat,  fill = "dryer"))+
  borders()+
  scale_fill_manual(values = c("#F46D43", "#74ADD1"), na.translate = F)+
...
ggpubr::ggarrange(plotlist = map_list,ncol = 2, nrow = 3, common.legend = T, legend = "bottom", labels = c("2030-2060", "2070-2100"), hjust = -3)
```
...
ggpubr::ggarrange(plotlist = map_list,ncol = 3, nrow = 2, common.legend = T, legend = "bottom", labels = c("2030-2060", "2070-2100"), hjust = -3)
```
...
```{r map-historical-changes-percent, fig.dim = c(8,4)}
map_list <- list()
...
```{r map-future-changes-percent, fig.dim = c(16,8)}
...
index <- paste(i, j, sep = " , ")
  b = pbreaks
g <- ggplot() + geom_raster(data = subset(cmip6s, period == i & model == j), aes(x=lon, y = lat,  fill = anomalies.percent))+
   borders()+
  binned_scale(aesthetics = "fill", breaks = b, palette = function(x) rev(colscale),
               guide = guide_legend(label.theme = element_text(angle = 0)))+
  labs(fill = "% change of AI index Compared to 1970-2000")+
  theme_void()+ylim(-55,90)+
  theme(legend.position = "bottom")
map_list[[index]] <- g
  }}
...
ggpubr::ggarrange(plotlist = map_list,ncol = 3, nrow = 2, common.legend = T, legend = "bottom", labels = c("SSP 2-4.5", "SSP 3-7.0", "SSP 5.85")) %>% annotate_figure(left = "2070-2100     2030-2060")
```
...
```{r map-historical-changes-dryerwetter, fig.dim = c(8, 4)}
...
```{r map-future-changes-dryerwetter, fig.dim = c(16,8)}
...
```{r map-future-changes-dry-catmaj-1, fig.dim = c(16,18)}
...
```{r map-historical-changes-wet-1, fig.dim = c(8, 4)}
map_list <- list()
...
```{r map-future-changes-wet-3, fig.dim = c(16,8)}
...
# Changes of variables over time and scenarios (reference period: 1970-2000)


### Temperature 

```{r map-historical-changes-quantiles-temperature, fig.dim = c(8,4)}
map_list <- list()
...
```{r map-future-changes-quantiles-temperature, fig.dim = c(16,8)}
...
```{r map-historical-changes-dryerwetter-catmaj, fig.dim = c(8, 4)}
...
```{r map-future-changes-dryerwetter-catmaj, fig.dim = c(16,8)}
...
```{r map-future-changes-dry-catmaj-2, fig.dim = c(16,18)}
...
```{r map-historical-changes-wet-2, fig.dim = c(8, 4)}
map_list <- list()
