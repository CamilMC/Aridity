...
breaks.martonne <- c(0,5,10,20,24,28,35,55,Inf)
cat.martonne <- c("[0,5]" = "Desert", "(5,10]"= "Dry", "(10,20]" = "Semi-dry","(20,24]" = "Mediterranean", "(24,28]" = "Semi-humid", "(28,35]" = "Humid","(35,55]" = "Very humid (a)", "(55,Inf]" = "Very humid (b)")
...
```{r all-indices}
all.indices <- world.df %>% select(c("lon","lat","pre","temp"))
...
all.indices$martonne.cat <- martonne.w$cat.AI
all.indices$martonne.catu <- all.indices$martonne.cat %>% revalue(c("Desert"="Hyperarid", "Dry"="Arid","Semi-dry"="Semi-arid","Mediterranean" = "Semi-arid", "Very humid (a)" = "Humid", "Very humid (b)" = "Humid"))
...
all.indices$max.cat <- with(select(all.indices, c("martonne.catu", "emberger.catu", "unesco.catu", "unep.catu")), which.max(table())) 
all.indices$agreement <-  with(all.indices, str_count(string = cat.AI, pattern = cat.AI.maj)/11)
