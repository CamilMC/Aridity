...
Il sert à décrire, en se basant sur des paramètres facilement accessibles, les zones climatiques telles qu’exposées par les phytogéographes. Malgré l’incohérence des unités, ce facteur montre bien que l’efficacité des précipitations augmente avec P et diminue avec T 6. 
...
martonne.civ$cat.AI <- martonne.civ$Am %>% cut(breaks.martonne, include.lowest = T) %>% revalue(cat.martonne)
...
g2c <- ggplot(martonne.civ)+ geom_raster(aes(x = lon, y = lat, fill = cat.AI))+
    borders(regions = "Ivory Coast")+
  labs(title = "De Martonne Aridity Index, Ivory Coast", fill = "")+
...
martonne.fra$cat.AI <- martonne.fra$Am %>% cut(breaks.martonne, include.lowest = T) %>% revalue(cat.martonne)
...
g2f <- ggplot(martonne.fra)+geom_raster(aes(x = lon, y = lat, fill = cat.AI))+
    borders(regions = "France")+
  labs(title = "De Martonne Aridity Index, France", fill = "")+
  scale_fill_manual(values = col.cat)+
  theme_void(base_size = 10)+theme(legend.position = "bottom")+guides(fill = guide_legend(nrow = 2))
...
martonne.w$cat.AI <- martonne.w$Am %>% cut(breaks.martonne, include.lowest = T) %>% revalue(cat.martonne)

ggplot(martonne.w)+geom_raster(aes(x = lon, y = lat, fill = cat.AI))+
  labs(title = "De Martonne Aridity Index", fill = "")+
  scale_fill_manual(values = col.cat)+theme_void(base_size = 10)+theme(legend.position = "bottom")+guides(fill = guide_legend(nrow = 2))
...
•	Mustafa et al 201810 : aridity trend in Kurdistan
•	Tabari 201411: aridity trends in Iran and impact on regional agriculture
•	Baltas 20068 : assessing climate in Northern Greece
...
Autres usages:
•	Ahma et al. 2018, Moral et al. 2016: De Martonne, Pinna, FAO indeces to assess the spatial distribution of aridity in Ethiopia and Spain.
•	Greve et al 201918 : how will the index evolve with climate change?
...
•	Kumar et Gautam20 2021 : frequency of droughts depending on aridity regions
