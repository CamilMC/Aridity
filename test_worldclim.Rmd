...
col.cat <- c("Deserts" = colorvec[2], "Arid"= colorvec[4],"Semi-arid"= colorvec[5], "Semi-humid" = colorvec[8], "Humid"= colorvec[10],"Hyper humid"= "powderblue")
```
...
col.baltas <- c("Desert" = colorvec[2],"Dry" = colorvec[4], "Semi-dry"= colorvec[5],"Meditarranean"= colorvec[6], "Semi-humid" = colorvec[8], "Humid"= colorvec[10],"Very humid (a)"= colorvec[11], "Very humid (b)" = "#004615")
...
ggplot(martonne.w)+geom_raster(aes(x = lon, y = lat, fill = cat.martonne))+
  labs(title = "De Martonne Aridity Index\n Martonne categories", fill = "")+
  scale_fill_manual(values = col.martonne)+theme_void(base_size = 10)+theme(legend.position = "bottom")+guides(fill = guide_legend(nrow = 1))
...
col.cat <- c("Hyperarid" = colorvec[2], "Arid"= colorvec[4],"Semi-arid"= colorvec[5], "Subhumid" = colorvec[8], "Humid"= colorvec[10],"Hyper-humid"= colorvec[11])
...
ggplot(subset(emberger.w, Em > -500 & Em < 2000))+geom_raster(aes(x = lon, y = lat, fill = Em))+
  labs(fill = "Em")+scale_fill_viridis_c(direction = -1)+theme_void(base_size = 10)+theme(legend.position = "bottom")
...
ggplot(emberger.w)+geom_raster(aes(x = lon, y = lat, fill = cat.AI))+
  labs(title = "Emberger Aridity Index", fill = "")+
  scale_fill_manual(values = col.cat)+theme_void(base_size = 10)+theme(legend.position = "bottom")+
...
Encore une fois, ne fonctionne pas si la température annuelle moyenne (ou tmax-tmin) est < 0: l'indice devient négatif. Les NA pourraient être "Cold"
...
col.cat <- c("Deserts" = colorvec[2], "Arid"= colorvec[4],"Semi-arid"= colorvec[5], "Semi-humid" = colorvec[8], "Humid"= colorvec[10],"Hyper-humid"= colorvec[11])
...
unesco.civ$tmin <-apply(dplyr::select(unesco.civ, c("tmean_01", "tmean_02", "tmean_03", "tmean_04", "tmean_05", "tmean_06", "tmean_07", "tmean_08", "tmean_09", "tmean_10", "tmean_11", "tmean_12")), 1, max, na.rm = T)
unesco.civ$tmin <- apply(dplyr::select(unesco.civ, c("tmean_01", "tmean_02", "tmean_03", "tmean_04", "tmean_05", "tmean_06", "tmean_07", "tmean_08", "tmean_09", "tmean_10", "tmean_11", "tmean_12")), 1, min, na.rm = T)
...
thorn.w <- mutate(world.df, 
                    ...
                    i_11 = (tmean_11/5)^1.514,
                    i_12 = (tmean_12/5)^1.514,
                    I = rowSums(across(starts_with("i_")), na.rm = T),
                    a = (6.751e-7)*I^3 - (7.7e-5)*I^2 + 0.01729*I + 0.49239,
                    e_01 = 16* geosphere::daylength(lat,"2000-01-15")/12* 31 /30*(10*tmean_01/I)^a, #Ep = 16 * L/12 * N/30 * (10T/I) ^a, L is length of daylight, N number of days in month
                    e_02 = 16* geosphere::daylength(lat,"2000-02-15")/12* 28 /30*(10*tmean_02/I)^a,
                    e_03 = 16* geosphere::daylength(lat,"2000-03-15")/12* 31 /30*(10*tmean_03/I)^a,
                    ...
                    e_11 = 16* geosphere::daylength(lat,"2000-11-15")/12* 30 /30*(10*tmean_11/I)^a,
                    e_12 = 16* geosphere::daylength(lat,"2000-12-15")/12* 31 /30*(10*tmean_12/I)^a,
                    ET = rowSums(across(starts_with("e_")), na.rm = T), # en mm
                    AI = pre/ET)
...
hg.w <- hg.w %>% mutate(psi = pi/180*lat, # latitude in radian
                    ...
                    MAI_11 = pre_11/e_11,
                    MAI_12 = pre_12/e_12,
                    ET = rowSums(across(starts_with("e_")), na.rm = T)*365, # en mm/daye_01/p_01
                    AI = pre/ET)
...
all.indices$lang.catu <- all.indices$lang.cat %>% revalue(c("Hyper humid" = "Humid"))

emberger.w <- readRDS("Worldclim/emberger.w.rds")
all.indices$emberger <- emberger.w$Em
all.indices$emberger.cat <- emberger.w$cat.AI
all.indices$emberger.catu <- all.indices$emberger.cat %>% revalue(c("Hyper-humid" = "Humid"))
...
saveRDS(all.indices, "Worldclim/all.indices.rds")
```
...
```{r compare-cat}
compAI <- function(AI1,AI2, df){ #AI must be one of "lang", "emberger", "martonne", "unesco","unep"
  
   if(AI2 == "unesco"){
  bp <- ggplot() + geom_rect(aes(xmin = -Inf, xmax = + Inf, ymin = 0, ymax = 0.03, fill = "1. Hyperarid"))+
  ...
  geom_rect(aes(xmin = -Inf, xmax = + Inf, ymin = 0.65, ymax = 3, fill = "5. Humid"))+
    scale_y_continuous(breaks = c(0.03,0.2,0.5,0.65), limits = c(0,3))
  ...
  geom_rect(aes(xmin = -Inf, xmax = + Inf, ymin = 0.5, ymax = 0.65, fill = "4. Semi-humid"))+
  geom_rect(aes(xmin = -Inf, xmax = + Inf, ymin = 0.65, ymax = 3, fill = "5. Humid"))+
  scale_y_continuous(breaks = c(0.03,0.2,0.5,0.75), limits = c(0,3))
  
  }else if(AI2 == "martonne"){
  bp <- ggplot()+
  ...
  geom_rect(aes(xmin = -Inf, xmax = + Inf, ymin = 40, ymax = 130, fill = "6. Forest"))+
  scale_y_continuous(breaks = c(5,10,20,30,40), limits = c(0,130))
  
  }else if(AI2 == "emberger"){
  bp <- ggplot()+
  ...
  geom_rect(aes(xmin = -Inf, xmax = + Inf, ymin = 150, ymax = 600, fill = "6. Hyperhumid"))+
  scale_y_continuous(breaks = c(10,45,70,110,150), limits = c(0,600))
  
  }else if(AI2 == "lang"){
  bp <- ggplot()+
  geom_rect(aes(xmin = -Inf, xmax = + Inf, ymin = 0, ymax = 20, fill = "1. Deserts"))+
  geom_rect(aes(xmin = -Inf, xmax = + Inf, ymin = 20, ymax = 40, fill = "2.Arid"))+
  geom_rect(aes(xmin = -Inf, xmax = + Inf, ymin = 40, ymax = 60, fill = "3. Semi-arid"))+
  geom_rect(aes(xmin = -Inf, xmax = + Inf, ymin = 60, ymax = 100, fill = "4. Semi-humid"))+
  geom_rect(aes(xmin = -Inf, xmax = + Inf, ymin = 100, ymax = 160, fill = "5. Humid"))+
  geom_rect(aes(xmin = -Inf, xmax = + Inf, ymin = 160, ymax = 200, fill = "6. Hyperhumid"))+
  scale_y_continuous(breaks = c(10,45,70,110,150), limits = c(0,200))
  }

  
  if(AI1 == "lang"){
  bip <- bp + geom_boxplot(data = filter(df, lang.cat == "Deserts"), aes(x="1. Deserts", y = .data[[AI2]]), col = "gray90", outlier.shape = NA)+
  geom_boxplot(data = filter(df, lang.cat == "Arid"), aes(x="2. Arid", y = .data[[AI2]]), outlier.shape = NA)+
  geom_boxplot(data = filter(df, lang.cat == "Semi-arid"), aes(x="3. Semi-arid", y = .data[[AI2]]), outlier.shape = NA)+
  geom_boxplot(data = filter(df, lang.cat == "Semi-humid"), aes(x="4. Semi-humid", y = .data[[AI2]]), outlier.shape = NA)+
  geom_boxplot(data = filter(df, lang.cat == "Humid"), aes(x="5. Humid", y = .data[[AI2]]), outlier.shape = NA)+
  geom_boxplot(data = filter(df, lang.cat == "Hyper humid"), aes(x="6. Hyper humid", y = .data[[AI2]]), outlier.shape = NA)
  
  }else if(AI1 == "emberger"){
  bip <- bp +  geom_boxplot(data = filter(df, emberger.cat == "Hyperarid"), aes(x="1. Hyperarid", y = .data[[AI2]]), col = "gray90", outlier.shape = NA)+
  geom_boxplot(data = filter(df, emberger.cat == "Arid"), aes(x="2. Arid", y = .data[[AI2]]), outlier.shape = NA)+
  geom_boxplot(data = filter(df, emberger.cat == "Semi-arid"), aes(x="3. Semi-arid", y = .data[[AI2]]), outlier.shape = NA)+
  geom_boxplot(data = filter(df, emberger.cat == "Subhumid"), aes(x="4. Dry subhumid", y = .data[[AI2]]), outlier.shape = NA)+
  geom_boxplot(data = filter(df, emberger.cat == "Humid"), aes(x="5. Humid", y = .data[[AI2]]), outlier.shape = NA)+
  geom_boxplot(data = filter(df, emberger.cat == "Hyper-humid"), aes(x="5. Hyper-humid", y = .data[[AI2]]), outlier.shape = NA)
  }else if(AI1 == "martonne"){
  bip <- bp +  geom_boxplot(data = filter(df, martonne.cat == "Desert"), aes(x="1. Hyperarid", y = .data[[AI2]]), col = "gray90", outlier.shape = NA)+
  geom_boxplot(data = filter(df, martonne.cat == "Arid"), aes(x="2. Arid", y = .data[[AI2]]), outlier.shape = NA)+
  geom_boxplot(data = filter(df, martonne.cat == "Semi-arid"), aes(x="3. Semi-arid", y = .data[[AI2]]), outlier.shape = NA)+
  geom_boxplot(data = filter(df, martonne.cat == "Temperate"), aes(x="4. Temperate", y = .data[[AI2]]), outlier.shape = NA)+
  geom_boxplot(data = filter(df, martonne.cat == "Humid"), aes(x="5. Humid", y = .data[[AI2]]), outlier.shape = NA)+
  geom_boxplot(data = filter(df, martonne.cat == "Forest"), aes(x="6. Forest", y = .data[[AI2]]), outlier.shape = NA)

  }else if(AI1 == "unep"){
  bip <- bp + geom_boxplot(data = filter(df, unep.cat == "Hyperarid"), aes(x="1. Hyperarid", y = .data[[AI2]]), col = "gray90", outlier.shape = NA)+
  geom_boxplot(data = filter(df, unep.cat == "Arid"), aes(x="2. Arid", y = .data[[AI2]]), outlier.shape = NA)+
  geom_boxplot(data = filter(df, unep.cat == "Semi-arid"), aes(x="3. Semi-arid", y = .data[[AI2]]), outlier.shape = NA)+
  geom_boxplot(data = filter(df, unep.cat == "Dry subhumid"), aes(x="4. Dry subhumid", y = .data[[AI2]]), outlier.shape = NA)+
  geom_boxplot(data = filter(df, unep.cat == "Humid"), aes(x="5. Humid", y = .data[[AI2]]), outlier.shape = NA)
  }else if(AI1 == "unesco"){
  bip <- bp +  geom_boxplot(data = filter(df, unesco.cat == "Hyperarid"), aes(x="1. Hyperarid", y = .data[[AI2]]), col = "gray90", outlier.shape = NA)+
  geom_boxplot(data = filter(df, unesco.cat == "Arid"), aes(x="2. Arid", y = .data[[AI2]]), outlier.shape = NA)+
  geom_boxplot(data = filter(df, unesco.cat == "Semi-arid"), aes(x="3. Semi-arid", y = .data[[AI2]]), outlier.shape = NA)+
  geom_boxplot(data = filter(df, unesco.cat == "Dry subhumid"), aes(x="4. Semi-humid", y = .data[[AI2]]), outlier.shape = NA)+
  geom_boxplot(data = filter(df, unesco.cat == "Humid"), aes(x="5. Humid", y = .data[[AI2]]), outlier.shape = NA)
  }
 
  beauty.bp <- bip +   
    scale_fill_viridis_d(guide = guide_legend(reverse = T, label.position = "left", title.hjust = 1))+
    labs(x=AI1, fill = AI2, y = "")+
    theme_minimal()+
    theme(legend.position = "left", legend.justification = "bottom", panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
```{r compare-plots}
all.indices <- readRDS("Worldclim/all.indices.rds")

  compAI("unesco","unesco", all.indices)
  compAI("unesco","unep", all.indices)
  compAI("unesco","martonne", all.indices)
  compAI("unesco","emberger", all.indices)
  compAI("unesco","lang", all.indices)
...
ipcc_regions.sf <- as.data.frame(ripcc_regions, xy = T) %>% setNames(c("lon","lat","Continent","Type","Name","Acronym")) %>% st_as_sf(coords = c("lon","lat"))
st_crs(ipcc_regions.sf) <- crs(ripcc_regions)
saveRDS(ipcc_regions.sf, "Worldclim/ipcc_regions.sf.rds")

all.indices <- readRDS("Worldclim/all.indices.rds")

all.ipcc <- all.indices %>% st_as_sf(coords = c("lon","lat"))
st_crs(all.ipcc) <- crs(ripcc_regions)

all.ripcc <- st_join(all.ipcc, ipcc_regions.sf, st_nearest_feature) 
all.ripcc.df <- all.ripcc %>% st_drop_geometry %>% cbind(select(all.indices, c("lon","lat")))

saveRDS(all.ripcc.df, "Worldclim/all.ripcc.df.rds")
``` 
...
ggplot(data = filter(ipcc_sf, Type %in% c("Land","Land-Ocean")))+
  geom_sf(aes(fill=Acronym))+
  scale_fill_viridis_d(option = "E", alpha = 0.5)+
 # geom_sf_text(aes(label = Name), size = 1, position = position_jitter(height = 3))+
  geom_sf_text(aes(label = Acronym), size = 1)+
  theme_void()+
  theme(legend.position = "none")
#ggsave("ipcc_regions.png", width = 12, height = 8, units = "cm", dpi = "retina")

ggplot(data = filter(ipcc_sf, Type %in% c("Land","Land-Ocean") & Acronym %in% c("NEU","WCE","MED")))+
  geom_sf(aes(fill=Acronym))+
  scale_fill_viridis_d(option = "E", alpha = 0.5)+
  geom_sf_text(aes(label = Name), size = 5, position = position_jitter(height = 3))+
  #geom_sf_text(aes(label = Acronym), size = 5)+
  theme_void()+
  theme(legend.position = "none")

ggplot(data = filter(ipcc_sf, Type %in% c("Land","Land-Ocean") & Acronym %in% c("WAF","CAF","WSAF","ESAF","SEAF","NEAF")))+
  geom_sf(aes(fill=Acronym))+
  scale_fill_viridis_d(option = "E", alpha = 0.5)+
  geom_sf_text(aes(label = Name), size = 5, position = position_jitter(height = 3))+
 # geom_sf_text(aes(label = Acronym), size = 5)+
...
ggplot(data = filter(ipcc_sf, Type %in% c("Land","Land-Ocean") & Acronym %in% c("NWS","NSA","SAM","NES","SES","SWS","SSA")))+
  geom_sf(aes(fill=Acronym))+
  scale_fill_viridis_d(option = "E", alpha = 0.5)+
  geom_sf_text(aes(label = Name), size = 5, position = position_jitter(height = 3))+
 # geom_sf_text(aes(label = Acronym), size = 5)+
...
ggplot(data = filter(ipcc_sf, Type %in% c("Land","Land-Ocean") & Acronym %in% c("NAU","CAU","EAU","SAU")))+
  geom_sf(aes(fill=Acronym))+
  scale_fill_viridis_d(option = "E", alpha = 0.5)+
  geom_sf_text(aes(label = Name), size = 5, position = position_jitter(height = 3))+
 # geom_sf_text(aes(label = Acronym), size = 5)+
...
## Compare Unesco and Martonne

### all regions

```{r compare-AI-regions}
all.ripcc.df <- readRDS("Worldclim/all.ripcc.df.rds")

for(i in unique(all.ripcc.df$Name)){
  df <- filter(all.ripcc.df, Name == i)
  print(compAI(AI1 = "unesco", AI2 = "martonne", df = df)+ggtitle(i))
}
```

### africa

```{r martonne-unesco-africa}
africa.lines <- grep(pattern = "Africa", x = all.ripcc.df$Name)
all.ripcc.df[grep("Africa", all.ripcc.df$Name), ]$Acronym %>% unique()

compAI(AI1 = "unesco", AI2 = "martonne", df = filter(all.ripcc.df, Acronym %in% c("WAF","CAF","WSAF","ESAF","SEAF","NEAF")))
```
### S. america

```{r martonne-unesco-samerica}
samerica.lines <- grep(pattern = "South-America", x = all.ripcc.df$Name)

compAI(AI1 = "unesco", AI2 = "martonne", df = filter(all.ripcc.df, Acronym %in% c("NWS","NSA","SAM","NES","SES","SWS","SSA")))
```

### Australia

```{r martonne-unesco-australia}
australia.lines <- grep(pattern = "Australia", x = all.ripcc.df$Name)

compAI(AI1 = "unesco", AI2 = "martonne", df = all.ripcc.df[australia.lines,])
```

### Europe

```{r martonne-unesco-europe}
compAI(AI1 = "unesco", AI2 = "martonne", df = filter(all.ripcc.df, Name %in% c("West&Central-Europe","Mediterranean")))
```


## Compare Unesco and Unep by regions

### All regions

```{r compare-unep-unesco-regions}
for(i in unique(all.ripcc.df$Name)){
  df <- filter(all.ripcc.df, Name == i)
  print(compAI(AI1 = "unesco", AI2 = "unep", df = df)+ggtitle(i))
}
```
...
```{r unep-unesco-africa}
compAI(AI1 = "unesco", AI2 = "unep", df = filter(all.ripcc.df, Acronym %in% c("WAF","CAF","WSAF","ESAF","SEAF","NEAF")))
```

### S. america

```{r unep-unesco-samerica}
compAI(AI1 = "unesco", AI2 = "unep", df = filter(all.ripcc.df, Acronym %in% c("NWS","NSA","SAM","NES","SES","SWS","SSA")))
```

### Australia

```{r unep-unesco-australia}
australia.lines <- grep(pattern = "Australia", x = all.ripcc.df$Name)

compAI(AI1 = "unesco", AI2 = "unep", df = all.ripcc.df[australia.lines,])
```

### Europe

```{r unep-unesco-europe}
compAI(AI1 = "unesco", AI2 = "unep", df = filter(all.ripcc.df, Name %in% c("West&Central-Europe","Mediterranean")))
```
