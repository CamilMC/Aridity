...
for(i in 1:nrow(calib)){
  ...
  catrefi <- ifelse(catrefi )
  cati <- calib$cat.AI[i] %>% as.character()
  calib$same.cat[i] <- ifelse(cati == catrefi, "y",
                              ifelse(cat!= catrefi, "n", ifelse(is.na(catrefi) == logical(0), "n")))
...
```{r pie-chart}
ggplot(table_calib)+geom_bar(aes(x = "", y = count, fill = value), width = 1, stat = "identity")+
  coord_polar("y", start = 0)+
  scale_fill_manual(values = col.cat)+
  facet_grid(cols = vars(source))+
  geom_text(aes(x = "", label = perc), color = "white")+
  labs(fill = "")+
...
ggplot(filter(calib.wide, same.cat == "n"))+geom_tile(aes(x = lon, y = lat, fill = cat.AI.cmip6, col = cat.AI.era5))+
  scale_fill_manual(values = col.cat)+
...
ggplot(filter(calib.wide, same.cat == "n"))+geom_tile(aes(x = lon, y = lat, fill = change.cat.era5))+
  scale_fill_discrete_diverging(palette = "Cork")+
  borders()+
  labs(fill = "Change of category between ERA5 and CMIP6")+
  ylim(-55,90)+
  theme_void()+
  theme(legend.position = "bottom")
