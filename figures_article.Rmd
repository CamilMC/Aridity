...
```{r map-diff-dryer-wetter, fig.dim = c(8,6)}
...
ggplot(filter(diffcatAI, cat.AI.wdc != cat.AI.cmip6)) + geom_raster(aes(x=lon, y = lat,  fill = wdc_cmip6))+
    ...
    theme(legend.position = "bottom")
```
```{r map-diff-categories, fig.dim = c(8,6)}
...
# Comparison of variables datasets 
...
# Aridity Index Hargreaves

```{r compute-hargreaves, eval = F}
