...
```{r scatterplot-diff, fig.dim = c(8,12)}
ggarrange(
...
ggplot(calib.wide,aes(x = Rn.wdc))+geom_point(aes( y = Rn.era5, col = "ERA5"), alpha =0.4, shape = 1)+
  ...
  stat_poly_eq(aes(y = Rn.cmip6, col = "CMIP6"), label.x = "right",label.y = "bottom") +
   geom_abline(slope = 1, intercept = 0)+
   labs(x="Rn, MJ/m2/day, WDC, °C", y = "Rn, MJ/m2/day", col = "Dataset")+
   scale_color_manual(values = c(colorvec[1], colorvec[4]))+
   theme_minimal()+
  theme(legend.position = "none"),
...
ncol = 2, nrow = 4, heights = c(1,1,1,0.2)
)
...
ggplot(filter(calib.wide, same.cat.wdc == "n"))+geom_tile(aes(x = lon, y = lat, fill = dryer.wdc))+
  scale_fill_manual(values = c(colorvec[4], colorvec[7]), na.translate = F)+
  borders()+
  labs(fill = "CMIP6 compared to WDC")+
  ylim(-55,90)+
  theme_void()+
...
```{r scatterplot-diff-2, fig.dim = c(8,12)}
ggarrange(
...
ggplot(filter(calib.wide, same.cat.era5 == "n"))+geom_tile(aes(x = lon, y = lat, fill = dryer.era5))+
  scale_fill_manual(values = c(colorvec[4], colorvec[7]), na.translate = F)+
  borders()+
  labs(fill = "CMIP6 compared to ERA5")+
  ylim(-55,90)+
  theme_void()+
  theme(legend.position = "bottom")
