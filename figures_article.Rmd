...
ndvi1 <- list.files(path = "NDVI", pattern = "ndvi.*\\.nc4$", full.names = T)
ndvi2 <- ndvi1 %>% lapply(FUN = monthlyComposite, varname = "ndvi") %>% raster::stack() %>% projectRaster(land_mask)
ndvi3 <- ndvi2 / 1e4
ndvi4 <- ndvi3 %>% mean(na.rm = T)
ndvi.df <- ndvi4 %>% as.data.frame(xy = T) %>% setNames(c("lon","lat","ndvi"))


ggplot(subset(ndvi.df, ndvi < -1))+geom_tile(aes(x=lon, y = lat, fill = ndvi))
ggplot(subset(ndvi.df, ndvi < -1 & ndvi > -3))+geom_tile(aes(x=lon, y = lat, fill = ndvi))

ndvi.df$ndvi[which(ndvi.df$ndvi < -3)] <- NA
write.table(ndvi.df, "NDVI/ndvi.df.txt")

ndvi.df <- read.table("NDVI/ndvi.df.txt")

flag1 <- ndvi1 %>% raster::stack(varname = "ndvi")
flag2 <- floor(flag1/10)/1000 
